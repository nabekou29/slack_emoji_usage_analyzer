...
class TestRateControl:
    ...
    @patch("emoji_usage.slack_client.settings")
    @patch("emoji_usage.slack_client.time")
    def test_respect_interval_no_wait_when_enough_time_passed(self, mock_time, mock_settings):
...
class TestSearchMessagesSafe:
    ...
    def test_429_retry_success(self, mock_sleep, mock_client, mock_respect_interval, mock_settings):
        """429エラーのリトライが成功するテスト"""
    ...
    @patch("emoji_usage.slack_client.client")
    @patch("emoji_usage.slack_client.time.sleep")
    def test_429_retry_failure(self, mock_sleep, mock_client, mock_respect_interval, mock_settings):
        """429エラーのリトライが失敗するテスト"""
    ...
    def test_non_429_error_propagation(self, mock_client, mock_respect_interval):
        ...
        mock_client.search_messages.side_effect = SlackApiError("Forbidden", mock_response_error)
...
class TestGetWorkspaceInfo:
    ...
    def test_workspace_info_failure_returns_none(
        ...
        mock_client.team_info.side_effect = SlackApiError("Rate limited", mock_response_429)
